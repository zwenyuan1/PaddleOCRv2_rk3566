...
int TextRec::Model_Infer(cv::Mat Input_Image, pair< vector<string>, double> &rec_res, double &times){
    ...
    // resize_img.convertTo(resize_img, CV_32FC3, e);//0-255 --> 0-1
    // cv::Mat temp_resize_img = resize_img.clone();
    ...
    // cout<<"input data type:"<<resize_img.type()<<endl;
    ...
    // ofstream file("./rknn_3566_rec_result.csv");
    // for (int i = 0; i < output_attrs[0].dims[1]; i++){
    //     for(int j=0; j< output_attrs[0].dims[2]; j++)
    //         file << outBlob[i*output_attrs[0].dims[2]+j]<<",";
    //     file<<'\n';
    // }
    // file.close();
