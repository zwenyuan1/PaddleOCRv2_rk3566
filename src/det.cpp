...
int TextDetect::Model_Infer(cv::Mat& Input_Image, vector<vector<vector<int>>> &boxes, double &times){  
    ...
    // cv::Mat ft_resize_img;
    // resize_img.convertTo(ft_resize_img, CV_32FC3, e);//0-255 --> 0-1

    // cout<<"ft_resize_img data type:"<<ft_resize_img.type()<<endl;

    // cv::Mat temp_resize_img = ft_resize_img.clone();

    // std::vector<cv::Mat> bgr_channels(3);
    // cv::split(temp_resize_img, bgr_channels);
    // for (auto i = 0; i < bgr_channels.size(); i++) {
    //     bgr_channels[i].convertTo(bgr_channels[i], CV_32FC1, 1.0 * scale_[i],
    //                             (0.0 - mean_[i]) * scale_[i]);
    // }
  
    // cv::merge(bgr_channels, resize_img);
    // cout<<"resize_img data type:"<<resize_img.type()<<endl;
    ...
    // write resize_img
    // cout<< resize_img.rows <<endl;
    // ofstream img_file0("./resize_img0.csv");
    // for(int i=0; i<resize_img.rows; i++){
    //     for(int j=0; j<resize_img.cols; j++){
    //         img_file0<< resize_img.at<Vec3f>(i,j)[0] << ',';
    //     }
    //     img_file0<< '\n';
    // }
    // img_file0.close();

    // ofstream img_file1("./resize_img1.csv");
    // for(int i=0; i<resize_img.rows; i++){
    //     for(int j=0; j<resize_img.cols; j++){
    //         img_file1<< resize_img.at<Vec3f>(i,j)[1] << ',';
    //     }
    //     img_file1<< '\n';
    // }
    // img_file1.close();

    // ofstream img_file2("./resize_img2.csv");
    // for(int i=0; i<resize_img.rows; i++){
    //     for(int j=0; j<resize_img.cols; j++){
    //         img_file2<< resize_img.at<Vec3f>(i,j)[2] << ',';
    //     }
    //     img_file2<< '\n';
    // }
    // img_file2.close();
    ...
    //int8_t* outBlob = (int8_t*)outputs[0].buf;
    float* outBlob = (float*)outputs[0].buf;
    // ofstream file("./rknn_3566_static_result.csv");
    // for (int i = 0; i < HEIGHT_; i++){
    //     for(int j=0; j< WIDTH_; j++)
    //         file << outBlob[i*WIDTH_+j]<<",";
    //     file<<'\n';
    // }
    // file.close();

    for (int i = 0; i < HEIGHT_ * WIDTH_; i++) {
        //pred[i] = deqnt_affine_to_f32(outBlob[i], out_zp, out_scale);
